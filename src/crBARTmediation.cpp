...
#define RHODRAW(a, b) RHOdraw[a][b]
  ...
  void crBARTmediation(int typeM,     // 1:continuous, 2:binary, 3:multinomial
    ...
    for(size_t postrep=0;postrep<total;postrep++) {
      ...
      //   uY[j]=gen.normal()*sd_uY_j*sqrt(1-pow(RHO[j],2))+
      //     (mu_uY_j + (RHO[j]*(sd_uY_j/sd_uM_j))*(uM[j]-mu_uM_j));
      // }
      ...
      if (typeM==1 && typeY==1) {
        for(size_t j=0; j<J; j++) {
          ...
          mu_uY_j = 0. + RHOtmp * (uMtmp - mu_uM_j);
          uYtmp = gen.normal() * sd_uY_j + mu_uY_j;
          for(size_t j=0; j<n_j; j++) {
            YMlik_curr += 
